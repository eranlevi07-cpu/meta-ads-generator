...
async function sendPhoto(token, chatId, imageDataOrUrl, caption) {
  if (imageDataOrUrl.startsWith('data:')) {
    ...
    if (!matches) {
      console.error('sendPhoto: invalid data URL');
      return null;
    }
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { brand, brief, ads = [], hrContent = [], imageUrls = [] } = JSON.parse(event.body);
    if (!brand || !brief) {
      return { statusCode: 400, headers: CORS_HEADERS, body: JSON.stringify({ error: 'Missing brand or brief' }) };
    }
    ...
    return { statusCode: 200, headers: CORS_HEADERS, body: JSON.stringify({ success: true, messageIds }) };
  } catch (error) {
